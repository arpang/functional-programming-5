...
object Extraction extends ExtractionInterface {
  ...
  def sparkLocateTemperatures(year: Year, stationsFile: String, temperaturesFile: String): DataFrame = {
    ...
    val station =
      Source.fromInputStream(getClass.getResourceAsStream(stationsFile), "utf-8").getLines()
        .map( _.split(","))
        .filter(_.length == 4).map( x ⇒ (x(0), x(1), x(2).toDouble, x(3).toDouble))
        .toList.toDF("stn", "wban", "latitude", "longitude")

    val temperature =
      Source.fromInputStream(getClass.getResourceAsStream(temperaturesFile), "utf-8").getLines()
        .map( _.split(","))
        .filter(_.length == 5).map( x ⇒ (x(0), x(1), x(2).toInt, x(3).toInt, x(4).toDouble))
        .toList.toDF("stn", "wban", "month", "day", "temperature")
...
object Test {
  def main(args: Array[String]): Unit = {
    ...
    val third = Visualization.predictTemperature(second, Location(5D, 5D))
    println(third)
  }
}
